...
const JobForm = ({ userId, allSkills }) => {
  ...
  useEffect(() => {
    if (!query) return;
    axios
      .post(`/employers/admins/${userId}/add_skill`, {
        skill: query,
      })
  ...
  return (
    <div className="container-login">
      <div className="wrap-login post-page">
        ...
        <form
          action={`/employers/admins/${userId}/jobs`}
          method="post"
          className="subheading-subtitles-posting"
        >
          ...
          <label htmlFor="adress:">
            Address (format: street address, city, state)
          </label>
          ...
          <div className="skills-checkboxes">
            ...
            {skills.map(({ name, id }, index) => (
              <span key={index}>
                ...
                <label style={{ marginRight: "1rem" }} htmlFor={id}>
                  {name}
                </label>
              </span>
            ))}
          ...
          >
            ...
            <input
              style={{ backgroundColor: "#2F78F3", height: "2.5rem" }}
              type="button"
              value="+ SKILL"
