...
const Jobs = ({
  ...
  function handleClick(e, jobId) {
    if (e.target.id === "unfav") {
      axios
        .post(`/applicants/users/${currentUser.id}/favorites`, { jobId })
        .then(({ data }) => {
          if (data) {
      ...
      axios
        .delete(`/applicants/users/${currentUser.id}/favorites`, {
          params: { jobId },
        })
        .then(({ data }) => {
          const newActiveFavorites = activeFavorites.filter(
            (fav) => fav !== jobId
          );
      ...
      {jobs.map(({ properties }, index) => (
        <div key={index} className="card m-0 h-auto w-100">
          <div className="card-body py-auto">
            ...
            <h6 className="card-text">{properties.company_name}</h6>
            <p className="card-text">{properties.description}</p>
            <p className="card-text">
              Skills required:{" "}
              {properties.skills.map(({ name }) => name).join(", ")} | Posted
              on: {properties.created_at.split("T")[0]}
            </p>
            {currentUser && !currentUser.employer && (
              <>
                <a
                  style={{ backgroundColor: "rgb(47, 120, 243)" }}
                  href={`/applicants/users/${currentUser.id}/${properties.id}/job_applications/new`}
                ...
                <button
                  id={activeFavorites.includes(properties.id) ? "fav" : "unfav"}
                  name={properties.id}
                  style={{
                    backgroundColor: "rgb(47, 120, 243)",
                    marginLeft: "1rem",
                  }}
                  className="btn btn-primary"
                  onClick={(e) => handleClick(e, properties.id)}
                >
                  Favorite
                </button>
              </>
